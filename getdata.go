...
func getUserData(email string) *User {
	...
	getUserInfo(u, "SDG%26E", email, UtilityBearerToken)
...
func getUserInfo(u *User, utility string, email string, token string) {
	var url = "https://utilityapi.com/api/v2/authorizations?utility=" + utility + "&customer_email=\"" + email + "\""
	log.Printf("fetched:\n %v", url)

	authorizations := makeRequest(url, "GET", token)
	array := authorizations["authorizations"].([]interface{})
	user := array[0].(map[string]interface{})
	...
	meterlist := getUserMeter(u.Useruid, token)
